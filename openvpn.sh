	...
	if [[ -e /etc/debian_version ]]; then
		...
		if [[ $ID == "debian" || $ID == "raspbian" ]]; then
			if [[ $VERSION_ID -lt 9 ]]; then
				echo "вљ пёЏ Your version of Debian is not supported."
				echo ""
			...
			if [[ $MAJOR_UBUNTU_VERSION -lt 16 ]]; then
				echo "вљ пёЏ Your version of Ubuntu is not supported."
				echo ""
		...
		if [[ $ID == "centos" ]]; then
			...
			if [[ ! $VERSION_ID =~ (7|8) ]]; then
				echo "вљ пёЏ Your version of CentOS is not supported."
				echo ""
		...
		if [[ $ID == "amzn" ]]; then
			...
			if [[ $VERSION_ID != "2" ]]; then
				echo "вљ пёЏ Your version of Amazon Linux is not supported."
				echo ""
...
function newClient() {
	...
	if [[ $CLIENTEXISTS == '1' ]]; then
		...
		1)
			...
			;;
		2)
			echo "вљ пёЏ You will be asked for the client password below вљ пёЏ"
